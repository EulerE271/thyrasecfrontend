...
export default function PerformanceGraph() {
  ...
  const averageDataPoints = (snapshots: Snapshots[], interval: string) => {
    ...
    snapshots.forEach((snapshot) => {
      // Ensure snapshot.Date is a Date object
      const dateObject = new Date(snapshot.Date);
      let dateKey = dateObject.toISOString().split("T")[0]; // Convert Date to string

      if (interval === "weekly") {
        dateKey = startOfWeek(dateObject)
          .toISOString()
          .split("T")[0];
      } else if (interval === "monthly") {
        dateKey = startOfMonth(dateObject)
          .toISOString()
          .split("T")[0];
      }
